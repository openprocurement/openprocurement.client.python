...
class ProfileServiceClient(APITemplateClient):
    ...
    def __init__(self, host_url='', api_version=None, auth_profile=None, headers=None):
        super(ProfileServiceClient, self).__init__(login_pass=auth_profile, headers=headers)
    ...
    def create_profile(self, data_profile):
        response = self.request("POST", path=self.prefix_path, json=data_profile)
