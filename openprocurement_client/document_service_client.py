...
class DocumentServiceClient(APITemplateClient):
    ...
    def files_processing(cls, files=None):
        # files variable is expected be in the following form:
    ...
    def document_upload(self, url, files, headers={}):
        # file_payload variable should be of the following form:
        # file_payload = {'file': (file_name, FileIO(file_path, 'rb'))}
        file_payload = files['file_payload']
        response_item = self.request(
